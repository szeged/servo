...
impl BluetoothManager {
    ...
    fn request_device(&mut self,
        ...
        if let Some(address) = self.select_device(matched_devices) {
            let device_id = match self.address_to_id.get(&address) {
                Some(id) => id.clone(),
                None => return drop(sender.send(Err(BluetoothError::Type(DEVICE_MATCH_ERROR.to_string())))),
            };
        ...
        return drop(sender.send(Err(BluetoothError::Type(DEVICE_MATCH_ERROR.to_string()))));
    }
    ...
    fn read_value(&mut self, id: String, sender: IpcSender<BluetoothResult<Vec<u8>>>) {
        ...
        let _ = sender.send(value.ok_or(BluetoothError::NotFound));
    }
