...
impl BluetoothManager {
    ...
    fn request_device(&mut self,
        ...
        for device in matched_devices {
            if let Ok(address) = device.get_address() {
                let message = Ok((address,
                                  device.get_name().ok(),
                                  device.get_class().ok(),
                                  device.get_vendor_id_source().ok(),
                                  device.get_vendor_id().ok(),
                                  device.get_product_id().ok(),
                                  device.get_device_id().ok(),
                                  device.get_appearance().ok(),
                                  match device.get_tx_power() {
                                      Ok(p) => Some(p as i8),
                                      Err(_) => None,
                                  },
                                  match device.get_rssi() {
                                      Ok(p) => Some(p as i8),
                                      Err(_) => None,
                                  },
                ));
    ...
    fn get_characteristic(&mut self,
        ...
        for characteristic in characteristics {
            if let Ok(uuid) = characteristic.get_uuid() {
                ...
                let message = Ok((uuid,
                                  characteristic.get_object_path(),
                                  properties.contains(BROADCAST),
                                  properties.contains(READ),
                                  properties.contains(WRITE_WITHOUT_RESPONSE),
                                  properties.contains(WRITE),
                                  properties.contains(NOTIFY),
                                  properties.contains(INDICATE),
                                  properties.contains(AUTHENTICATED_SIGNED_WRITES),
                                  properties.contains(RELIABLE_WRITE),
