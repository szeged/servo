...
impl WebGLFramebuffer {
    ...
    pub fn update_status(&self) {
        ...
        for (attachment, constraints) in attachments.iter().zip(&attachment_constraints) {
            if let Err(errnum) =
                self.check_attachment_constains(attachment, constraints, &mut fb_size)
            {
        ...
        for attachment in self.colors.iter() {
            ...
            if let Err(errnum) =
                self.check_attachment_constains(&*attachment, color_constraints, &mut fb_size)
            {
                return self.status.set(errnum);
            }
        }
    ...
    pub fn texture2d(
        ...
        assert!(target == self.target.get().unwrap());
        self.upcast::<WebGLObject>()
            .context()
            .send_command(WebGLCommand::FramebufferTexture2D(
                target, attachment, textarget, tex_id, level,
            ));
