...
impl GPUDeviceMethods for GPUDevice {
    ...
    fn CreateComputePipeline(&self, descriptor: &GPUComputePipelineDescriptor) -> DomRoot<GPUComputePipeline> {
        let pipeline = descriptor.parent.layout.id();
        let program = descriptor.computeStage.module.id();
        let entry_point = descriptor.computeStage.entryPoint.as_bytes().iter().map(|c| *c as i8).collect::<Vec<i8>>();
        let id = self
            .global()
