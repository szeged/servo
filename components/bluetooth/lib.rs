...
impl BluetoothManager {
        ...
        while let Ok(msg) = self.receiver.recv() {
            match msg {
                ...
                BluetoothRequest::GetGATTChildren(id, uuid, single, child_type, sender) => {
                    self.get_gatt_children(id, uuid, single, child_type, sender)
                },
    ...
    fn get_gatt_children(&mut self,
        ...
        match child_type {
            GATTType::PrimaryService => {
                // Step 5.
                if !self.cached_devices.contains_key(&id) || !self.address_to_id.values().any(|v| v == &id) {
                ...
                return drop(sender.send(Ok(BluetoothResponse::GetPrimaryServices(services_vec, single))));
            },
            GATTType::Characteristic => {
                // Step 5.
                if !self.cached_services.contains_key(&id) || !self.service_to_device.contains_key(&id) {
                    return drop(sender.send(Err(BluetoothError::InvalidState)));
                }
                // Step 6.
                ...
                return drop(sender.send(Ok(BluetoothResponse::GetCharacteristics(characteristics_vec, single))));
            },
            GATTType::IncludedService => {
                ...
                return drop(sender.send(Ok(BluetoothResponse::GetIncludedServices(services_vec, single))));
            },
            GATTType::Descriptor => {
                // Step 5.
                ...
                return drop(sender.send(Ok(BluetoothResponse::GetDescriptors(descriptors_vec, single))));
            },
        }
    }
    ...
    fn read_value(&mut self, id: String, sender: IpcSender<BluetoothResponseResult>) {
        // (Characteristic) TODO: Step 3.
        // (Descriptor)     TODO: Step 3.
    ...
    fn write_value(&mut self, id: String, value: Vec<u8>, sender: IpcSender<BluetoothResponseResult>) {
        // (Characteristic) TODO: Step 5.
        // (Descriptor)     TODO: Step 5.
