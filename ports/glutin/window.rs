...
pub struct Window {
    kind: WindowKind,
    dxgi_window: RefCell<Option<ResultWindow>>,
...
impl Window {
    ...
    pub fn new(is_foreground: bool,
               ...
               parent: Option<winit::WindowId>) -> Rc<Window> {
...
impl WindowMethods for Window {
    ...
    fn present(&self) {
        match self.kind {
            WindowKind::Window(..) => {
                if let Some(ref window) = *self.dxgi_window.borrow() {
                    if let Some(ref win) = *window {
                        win.swap_buffers(0);
                    }
                } else {
                    warn!("D3D11 window is not set!");
                }
            }
            WindowKind::Headless(..) => {}
        }
    ...
    fn set_dxgi_window(&self, window: Option<ResultWindow>) {
        *self.dxgi_window.borrow_mut() = window;
    }
}
